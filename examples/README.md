...
`out`), `-css` (stylesheet to copy alongside the HTML; default
`examples/notion.css`), `-js` (script to copy alongside the HTML; default
`examples/notion.js`), `-max-assets`.
...
Open <http://localhost:8080/> and paste a public Notion page URL.

## `fullapp` — starter app

Starts a fuller HTTP app intended as a copyable starting point for real users.
It embeds the sample renderer CSS and sample interaction JS with `go:embed`,
loads KaTeX CSS/JS from a pinned CDN URL for equation hydration, and stores
downloaded Notion assets on disk before re-serving them under `/assets/`.
...
- **Styling.** [`notion.css`](notion.css) is a sample stylesheet for the
  `notion-*` classes the renderer emits. It `@import`s web fonts from public
  CDNs; self-host or remove those imports if you prefer. The full app uses a
  copied, system-font version and loads KaTeX itself from a pinned CDN URL.
